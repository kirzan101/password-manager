...
class CredentialFormRequest extends FormRequest
{
    ...
    {
        $user = $this->user();
        $routeParam = $this->route('credentials');

        // If the route parameter is numeric, treat it as an existing model (update)
        if (is_numeric($routeParam)) {
            $model = Credential::find($routeParam);
            return $user->can(Helper::ACTION_TYPE_UPDATE, $model);
        }

        if ($routeParam instanceof Credential) {
            // If the route parameter is an instance of Credential, treat it as an existing model (update)
            return $user->can(Helper::ACTION_TYPE_UPDATE, $routeParam);
        }

        // Otherwise, this is a create request
        return $user->can(Helper::ACTION_TYPE_CREATE, Credential::class);
    }
